...
using Hylo;
using Hylo.EnumerableExtensions;
...
namespace CloudStreams.Dashboard.StateManagement
{
        ...
        : IStoreFactory
    {
        ...
        private static readonly MethodInfo AddFeatureMethod = typeof(IStoreExtensions).GetMethods().First(m => m.Name == nameof(IStoreExtensions.AddFeature) && m.GetParameters().Length == 2);
        ...
        protected virtual LambdaExpression BuildReducerLambda(Type stateType, Type actionType, MethodInfo reducerMethod)
        {
            ...
            if (reducerMethod == null) throw new ArgumentNullException(nameof(reducerMethod));
        ...
        {
            if (actionType == null) throw new ArgumentNullException(nameof(actionType));
            if (effectMethod == null) throw new ArgumentNullException(nameof(effectMethod));
            var actionParam = Expression.Parameter(actionType, "action");
