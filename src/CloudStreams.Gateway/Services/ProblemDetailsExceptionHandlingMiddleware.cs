    ...
    where TResult : IOperationResult
{
    ...
    public virtual async Task<TResult> HandleAsync(TRequest request, RequestHandlerDelegate<TResult> next, CancellationToken cancellationToken = default)
    {
        ...
        {
            if (!this.TryCreateErrorResponse(ex.Problem, out var response)) throw;
            return response;
        }
    }
    ...
    /// <param name="problem">The <see cref="Neuroglia.ProblemDetails"/> to create the new <see cref="IOperationResult"/> for</param>
    /// <returns>A new error <see cref="IOperationResult"/></returns>
    protected virtual bool TryCreateErrorResponse(Neuroglia.ProblemDetails problem, out TResult result)
    {
        Type responseType;
