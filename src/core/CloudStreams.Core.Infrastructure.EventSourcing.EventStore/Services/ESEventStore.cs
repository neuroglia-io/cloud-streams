...
/// <summary>
/// Represents the <see href="https://www.eventstore.com/">EventStore</see> based implementation of the <see cref="IEventStore"/> interface
/// </summary>
public class ESEventStore
    : BackgroundService, IEventStore
{
    ...
    public ESEventStore(ILoggerFactory loggerFactory, EventStoreClient streams, EventStoreProjectionManagementClient projections)
    {
    ...
    /// <inheritdoc/>
    public virtual async Task<Data.StreamMetadata> GetStreamMetadataAsync(CancellationToken cancellationToken = default)
    {
        try
        {
    ...
    public virtual async IAsyncEnumerable<string> ListPartitionIdsAsync(CloudEventPartitionType partitionType, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            partitionsIds = await this.Projections.GetResultAsync<List<string>>(EventStoreProjections.CloudEventPartitionsMetadataPrefix + typeName, cancellationToken: cancellationToken).ConfigureAwait(false);
        }
        catch { }
    ...
    public virtual async IAsyncEnumerable<CloudEventRecord> ReadAsync(StreamReadDirection readDirection, long offset, ulong? length = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            readResult = this.Streams.ReadStreamAsync(readDirection.ToDirection(), streamName, position, (long)(length ?? long.MaxValue), resolveLinkTos, cancellationToken: cancellationToken);
            if(await readResult.ReadState.ConfigureAwait(false) == ReadState.StreamNotFound) yield break;
        }
        catch (StreamNotFoundException)
        {
            yield break;
    ...
    protected virtual async Task SetupProjectionsAsync(CancellationToken cancellationToken = default)
    {
        ...
        stream = typeof(ESEventStore).Assembly.GetManifestResourceStream(string.Join('.', typeof(EventStoreProjections).Namespace, "Assets", "Projections", "bysource.js"))!;
        ...
        stream = typeof(ESEventStore).Assembly.GetManifestResourceStream(string.Join('.', typeof(EventStoreProjections).Namespace, "Assets", "Projections", "bysubject.js"))!;
        streamReader = new StreamReader(stream);
        query = await streamReader.ReadToEndAsync(cancellationToken).ConfigureAwait(false);
        streamReader.Dispose();
        ...
        stream = typeof(ESEventStore).Assembly.GetManifestResourceStream(string.Join('.', typeof(EventStoreProjections).Namespace, "Assets", "Projections", "bycausationid.js"))!;
        streamReader = new StreamReader(stream);
        query = await streamReader.ReadToEndAsync(cancellationToken).ConfigureAwait(false);
        streamReader.Dispose();
        ...
        stream = typeof(ESEventStore).Assembly.GetManifestResourceStream(string.Join('.', typeof(EventStoreProjections).Namespace, "Assets", "Projections", "partitionids.js.tmpl"))!;
        streamReader = new StreamReader(stream);
        query = await streamReader.ReadToEndAsync(cancellationToken).ConfigureAwait(false);
        streamReader.Dispose();
    ...
    protected virtual async Task<CloudEventRecord> DeserializeResolvedEventAsync(ResolvedEvent e, CancellationToken cancellationToken)
    {
        var data = e.Event.ContentType switch
        {
            ...
            "application/x-yaml" or "text/yaml" or "text/x-yaml" => Serializer.Yaml.Deserialize<object>(Encoding.UTF8.GetString(e.Event.Data.Span)),
